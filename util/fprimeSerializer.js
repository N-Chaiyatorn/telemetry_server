...
function fprimeSerializer(usrCommand, target) {
    ...
    */
    ...
    let opcode = usrCommand['opCode'];
    let types = commandDict[opcode.toString()]['arguments'].map((a) => a);
    ...
    let argBufferArr = usrCommand['arguments'].map(function (a, i) {
        if (types[i] == 'string') {
            ...
            let enumArray = types[i][1];
            let enumVal = ((enumArray) => {
    ...
    let commandBuffArray = [header, numBuff(length, 32, 'U'), numBuff(desc, 32, 'U'), numBuff(opcode, 32, 'U'), commandBufferArgs];

    let checksumBuff = checksumGenerator(concatBuffs(commandBuffArray));
